...
export class AddPersonFormComponent {
  ...
  validationMessageLabel!: ElementRef<HTMLParagraphElement>;
  ...
  validate(): boolean {
    if (this.nameInput.nativeElement.value.length === 0) {
      ValidatorView.showFailure(
        this.validationMessageLabel.nativeElement,
        'Nazwa osoby nie może być pusta'
      );
      return false;
    }
    ValidatorView.showSuccess(
      this.validationMessageLabel.nativeElement,
